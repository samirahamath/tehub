...
<head>
  ...
  <title>Admin Dashboard · THE EXPERT HUB Management</title>
...
<body>
  ...
  <header class="admin-header">
    <a href="admin_dashboard.php" class="admin-brand">
      THE EXPERT HUB <span class="badge-admin">CONTROL PANEL</span>
    </a>
    <nav class="admin-nav">
    ...
    <div>
      <span
        style="font-size: 13px; color: #a0aec0; margin-right: 15px;"><?= htmlspecialchars($_SESSION['admin_user']['email']) ?></span>
      <a href="admin_logout.php" style="color: #ef4444; text-decoration: none; font-size: 13px; font-weight: 600;">Sign
        Out</a>
    </div>
  </header>
    ...
    <?php if ($tab === 'services'): ?>
      <div class="panel">
        ...
        <p style="color: #8a94a6; font-size: 14px; margin-bottom: 20px;">
          Changes made here instantly update the rates displayed on the main <a href="services.php" target="_blank"
            style="color: var(--accent);">Services Page</a>.
        </p>
          ...
          <tbody>
            <?php foreach ($services_list as $s): ?>
              <tr>
                ...
                <td><span
                    style="color: var(--accent); font-weight: 700;"><?= htmlspecialchars($s['price_prefix']) ?><?= number_format($s['price']) ?></span>
                </td>
                ...
                <td>
                  <button onclick='editService(<?= json_encode($s, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
                    class="btn-action btn-sm">Edit</button>
        ...
        <table class="table-custom">
          ...
          <tbody>
            <?php if (empty($orders_list)): ?>
              <tr>
                <td colspan="7" style="text-align:center; color:#8a94a6; padding: 30px;">No orders recorded yet.</td>
              </tr>
            <?php else: ?>
        ...
        <table class="table-custom">
          ...
          <tbody>
            <?php if (empty($clients_list)): ?>
              <tr>
                <td colspan="4" style="text-align:center; color:#8a94a6; padding: 30px;">No registered clients found.</td>
              </tr>
            <?php else: ?>
          ...
          <div class="form-grid">
            <div class="form-group-full">
              ...
              <select name="payment_gateway_provider" class="form-select">
                <option value="stripe" <?= get_setting('payment_gateway_provider') === 'stripe' ? 'selected' : '' ?>>Stripe
                  Payments</option>
                <option value="paypal" <?= get_setting('payment_gateway_provider') === 'paypal' ? 'selected' : '' ?>>PayPal
                  Express</option>
                <option value="bank" <?= get_setting('payment_gateway_provider') === 'bank' ? 'selected' : '' ?>>Manual Wire
                  Transfer / UPI</option>
              </select>
            </div>
            <div>
              ...
              <input type="text" name="stripe_public_key" class="form-input"
                value="<?= htmlspecialchars(get_setting('stripe_public_key')) ?>" placeholder="pk_test_..." />
            </div>
            <div>
              <label class="form-label">Stripe Secret Key</label>
              <input type="password" name="stripe_secret_key" class="form-input"
                value="<?= htmlspecialchars(get_setting('stripe_secret_key')) ?>" placeholder="sk_test_..." />
            </div>
            <div class="form-group-full">
              <label class="form-label">PayPal Client ID</label>
              <input type="text" name="paypal_client_id" class="form-input"
                value="<?= htmlspecialchars(get_setting('paypal_client_id')) ?>" placeholder="Client ID string..." />
            </div>
            <div>
              <label class="form-label">UPI Payment ID (Optional)</label>
              <input type="text" name="upi_id" class="form-input" value="<?= htmlspecialchars(get_setting('upi_id')) ?>"
                placeholder="tehub@upi" />
            </div>
            <div>
              <label class="form-label">Bank Details / Instructions</label>
              <textarea name="bank_details" class="form-textarea"
                placeholder="Account Number, Swift Code, Bank Name..."><?= htmlspecialchars(get_setting('bank_details')) ?></textarea>
            </div>
          </div>
          ...
          <div class="form-grid">
            <div>
              ...
              <input type="text" name="smtp_host" class="form-input"
                value="<?= htmlspecialchars(get_setting('smtp_host', 'smtp.gmail.com')) ?>"
                placeholder="smtp.gmail.com" />
            </div>
            <div>
              <label class="form-label">SMTP Port</label>
              <input type="text" name="smtp_port" class="form-input"
                value="<?= htmlspecialchars(get_setting('smtp_port', '587')) ?>" placeholder="587" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" name="smtp_user" class="form-input"
                value="<?= htmlspecialchars(get_setting('smtp_user')) ?>" placeholder="user@domain.com" />
            </div>
            <div>
              <label class="form-label">SMTP Password</label>
              <input type="password" name="smtp_pass" class="form-input"
                value="<?= htmlspecialchars(get_setting('smtp_pass')) ?>" placeholder="••••••••" />
            </div>
            <div>
              <label class="form-label">Sender From Email</label>
              <input type="email" name="smtp_from_email" class="form-input"
                value="<?= htmlspecialchars(get_setting('smtp_from_email', 'noreply@tehub.com')) ?>"
                placeholder="noreply@tehub.com" />
            </div>
          </div>
          <div style="margin-top:20px;">
    ...
    <div class="modal-content">
      ...
      <form method="POST">
        ...
        <div class="form-group" style="margin-bottom:12px;">
          ...
          <textarea name="short_description" id="svc_short_description" class="form-textarea"
            placeholder="Describe the target audience and purpose..."></textarea>
        </div>
        <div style="display:flex; gap:12px; margin-bottom:12px;">
          <div style="flex:1;">
            <label class="form-label">Price (₹)</label>
            <input type="number" step="0.01" name="price" id="svc_price" class="form-input" required
              placeholder="10000" />
          </div>
          <div style="flex:1;">
        ...
        <div style="display:flex; gap:12px; margin-bottom:12px;">
          <div style="flex:1;">
            ...
            <input type="text" name="duration_info" id="svc_duration_info" class="form-input" placeholder="1 week" />
          </div>
          <div style="flex:1;">
        ...
        <div class="form-group" style="margin-bottom:20px;">
          ...
          <textarea name="features" id="svc_features" class="form-textarea" style="min-height:120px;"
            placeholder="1 Lead architect&#10;Clean Next.js frontend&#10;3-month warranty"></textarea>
        </div>
        <div style="display:flex; justify-content:flex-end; gap:10px;">
          <button type="button" onclick="closeServiceModal()" class="btn-action btn-danger"
            style="background:#4b5563;">Cancel</button>
